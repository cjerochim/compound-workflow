...
- Flags (by convention in `$ARGUMENTS`):
  - `--dry-run`: print resolved target + planned file/JSON ops; **no writes**
...
1. **Resolve host repo root (`HOST_ROOT`).**
   ...
   - Print the resolved `HOST_ROOT` as an **absolute path** before any writes.
   - Safety checks (enforce, not just doc):
     - Refuse obvious template targets (a dir containing `src/.agents` and `src/AGENTS.md`).
     - Host marker validation: require `HOST_ROOT/.git` unless `--force` is explicitly present.
...
2. **Resolve clone root (`CLONE_ROOT`).**
   ...
   - Print the resolved `CLONE_ROOT` as an **absolute path**.

3. **Copy and merge into host.**
   - Copy `CLONE_ROOT/src/.agents` → `HOST_ROOT/.agents` (replace existing).
     - Critical: avoid `.agents/.agents` nesting; replace the destination directory, do not copy the source dir into an existing `.agents/`.
     - Always print absolute paths that will be modified: `HOST_ROOT/.agents/**` and (maybe) `HOST_ROOT/AGENTS.md`.
   - **AGENTS.md:**
...
4. **Sync OpenCode configuration in the host repo.**
   ...
   - 4.1 Discover commands: glob `HOST_ROOT/.agents/commands/**/*.md`
     - command id: **frontmatter `invocation` (preferred)** else `name` else basename
   ...
   - 4.7 Prefer the deterministic script (supports `--dry-run` and prints idempotency summary): run
     - `node HOST_ROOT/.agents/scripts/sync-opencode.mjs --root HOST_ROOT [--dry-run]`

5. **Confirm.**
   - List what was copied and that `opencode.json` was updated.
   - Print idempotency summary: “0 changes needed” vs “updated N managed entries.”
   - Run post-sync self-check and surface missing registrations / missing frontmatter:
     - `node HOST_ROOT/.agents/scripts/self-check.mjs --root HOST_ROOT`
   - Suggest running `opencode debug config` in the host repo to verify.
